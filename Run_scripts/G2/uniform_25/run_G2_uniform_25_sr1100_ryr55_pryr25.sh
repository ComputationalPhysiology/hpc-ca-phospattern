...
GEOMETRY=G2_uniform_25
PARAMETERS=parameters_sr1100_ryr55_pryr25

RUNNR=$(printf "%04d" $PJM_BULKNUM)
CASENAME="result_"$GEOMETRY"_sr1100_ryr55_pryr25_"$RUNNR
...
then
  ...
  MPI_ARGS="--mpi_dist 2 2 1"
  ADDITIONAL_ARGS="-f -X 1 -s Ca -O p_ryr "
  VAR="$(($n-$n_RyR))"
  APP="calcium_sparks"
fi
